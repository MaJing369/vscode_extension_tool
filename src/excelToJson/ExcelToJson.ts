import { readdirSync, readFileSync, stat, statSync, writeFile } from "fs";
...
export class ExcelToJson {
    ...
    private static _instance: ExcelToJson;
    private _fileMap: Map<string, string> = new Map();
    private _filterArr: string[] = [];
     ...
     */
    onMessage(panel: WebviewPanel, msgData: { type: number, list?: string[], merge?: boolean }): void {
    ...
    private openFile(panel: WebviewPanel): void {
        window.showOpenDialog({ defaultUri: Uri.file(join(__dirname, "..", "..", "..")), canSelectFiles: true, canSelectFolders: false })
            .then(result => {
                this.clearDate(panel);
                if (result) {
                    const readPath = result[0].fsPath;
                    if (!this.fileTest(readPath)) {
                        window.showInformationMessage("请选择excel文件")
                        return;
                    }
                    let fileName = basename(readPath);
    ...
    private openDir(panel: WebviewPanel): void {
        window.showOpenDialog({ defaultUri: Uri.file(join(__dirname, "..", "..", "..")), canSelectFiles: false, canSelectFolders: true }).then(result => {
            this.clearDate(panel);
            if (result) {
                let fileList = readdirSync(result[0].fsPath);
                ...
                fileList.forEach(fileName => {
                    ...
                    if (fileName.indexOf("~$") >= 0 || !this.fileTest(fileName)) return;
                    let readPath = result[0].fsPath + "\\" + fileName;
                    let fileStat = statSync(readPath);
                    if (fileStat.isFile()) {
     ...
     * @description: 过滤文件处理
     */
    private setFilterFile(panel: WebviewPanel, filePath: string): void {
    ...
    private getFileData(path: string, panel: WebviewPanel, encoding: string = "utf-8"): string | null {
        ...
        try {
            ...
            window.showInformationMessage("文件读取失败：" + path)
        }
        return rd;
    }

    private fileTest(filePath: string): boolean {
        if (filePath.indexOf(".xlsx") < 0 && filePath.indexOf(".xls") < 0 && filePath.indexOf(".filter") < 0) return false;
        return true;
    }

    private addFileLi(panel: WebviewPanel, fileName: string): void {
    ...
    private addFilterLi(panel: WebviewPanel, fileName: string): void {
        panel.webview.postMessage({
            command: 'filterList',
            text: fileName
        });
    ...
    private clearDate(panel: WebviewPanel): void {
        this._fileMap.clear();
        this._filterArr = [];
        panel.webview.postMessage({
            command: 'clearList'
     ...
     * @param {*} merge
     */
    private outPutFile(panel: WebviewPanel, selectList?: string[], merge?: boolean): void {
        window.showOpenDialog({ defaultUri: Uri.file(join(__dirname, "..", "..", "..")), canSelectFiles: false, canSelectFolders: true }).then(result => {
            if (result) {
                ...
                if (merge) { //合并为一个json
                    ...
                    writeFile(result[0].fsPath + "\\config.json", JSON.stringify(jsonData), () => {
                        window.showInformationMessage("json导出完成");
     ...
     */
    private saveFiles(files: string[][], idx: number, savePath: string): void {
        let d = files[idx];
        writeFile(savePath + "\\" + d[0] + ".json", JSON.stringify(d[1]), () => {
            if (idx >= files.length - 1) {
                window.showInformationMessage("json导出完成");
                this._isOutPut = false
            } else this.saveFiles(files, idx + 1, savePath);
        })
    }
     ...
     * @return {*}
     */
    private analysisXlsx(selectList?: string[]): Map<string, string> {
        let xlsxDates: Map<string, any> = new Map();
        this._fileMap.forEach((itemFile, fileName) => {
            if (!selectList || selectList.includes(fileName)) {
                let xlsxDate = parse(itemFile);
                if (xlsxDate && xlsxDate.length > 0) {
                    xlsxDate.forEach((value, index) => {
                        if (value.data.length > 0) {
                            let jsonDataArr = [];
                            let yData: any[] = value.data;
                            ...
                            if (jsonDataArr.length > 0) {
                                ...
                                if (value.name.indexOf("Sheet") >= 0) {
                                    ...
                                    if (xlsxDate.length === 1) {
                                        ...
                                        window.showInformationMessage(fileName + " : " + value.name + " 命名错误请检查")
                                        return;
                                    }
                                }
    ...
    static get instance(): ExcelToJson {
